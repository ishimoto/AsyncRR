...
import er.extensions.formatters.*;

public class AjaxPTUtils {

	private final static Logger log = Logger.getLogger(AjaxPTUtils.class);
...
//	}
	 ...
	 */
	public static AjaxPTResponse createResponse(WORequest request, WOContext context) {
		AjaxPTResponse response = null;
		if (context != null && context.response() != null) {
			WOResponse existingResponse = context.response();
			if (existingResponse instanceof AjaxPTResponse) {
				response = (AjaxPTResponse) existingResponse;
			} else {
				response = new AjaxPTResponse(request, context);
...
//		AjaxPTUtils.addScriptResourceInHead(context, response, "Ajax", fileName);
//	}
...
//		AjaxPTUtils.addStylesheetResourceInHead(context, response, "Ajax", fileName);
//	}
...
//				ERXResponseRewriter.TagMissingBehavior.Top);
...
//		// PROTOTYPE MISC
//		if (!(WOApplication.application().resourceManager() instanceof ERXResourceManager)
//				&& "Ajax".equals(framework)
//				&& "scriptaculous.js".equals(fileName)
//				&& !(context.request() == null || context.request() != null && context.request().isUsingWebServer()
//						&& !WOApplication.application()._rapidTurnaroundActiveForAnyProject())) {
//			boolean enqueueIfTagMissing = !AjaxPTUtils.isAjaxRequest(context.request());
//			ERXResponseRewriter.addResourceInHead(response, context, framework, "builder.js", startTag, endTag,
//					ERXResponseRewriter.TagMissingBehavior.Top);
//			ERXResponseRewriter.addResourceInHead(response, context, framework, "effects.js", startTag, endTag,
//					ERXResponseRewriter.TagMissingBehavior.Top);
//			ERXResponseRewriter.addResourceInHead(response, context, framework, "dragdrop.js", startTag, endTag,
//					ERXResponseRewriter.TagMissingBehavior.Top);
//			ERXResponseRewriter.addResourceInHead(response, context, framework, "controls.js", startTag, endTag,
//					ERXResponseRewriter.TagMissingBehavior.Top);
//			ERXResponseRewriter.addResourceInHead(response, context, framework, "slider.js", startTag, endTag,
//					ERXResponseRewriter.TagMissingBehavior.Top);
//		}
//	}
...
//	}
	...
	public static boolean shouldHandleRequest(WORequest request, WOContext context, String containerID) {
		...
		if (containerID != null) {
			if (AjaxPTResponse.isAjaxUpdatePass(request)) {
				updateContainerID = AjaxPTUpdateContainer.updateContainerID(request);
			}
		}
		boolean shouldHandleRequest = elementID != null
				&& (elementID.equals(senderID) || (containerID != null && containerID.equals(updateContainerID)) || elementID
						.equals(ERXAjaxApplication.ajaxSubmitButtonName(request)));
...
//		AjaxPTUtils.updateMutableUserInfoWithAjaxInfo(context.response());
//	}
...
//	}
	...
	public static WOResponse javascriptResponse(String javascript, WOContext context) {
		...
		AjaxPTResponse response = AjaxPTUtils.createResponse(request, context);
	...
	public static void appendScript(WOContext context, String script) {
		AjaxPTUtils.appendScript(context.request(), context.response(), script);
	}
	...
	public static void appendScriptHeaderIfNecessary(WORequest request, WOResponse response) {
		if (AjaxPTUpdateContainer.hasUpdateContainerID(request)) {
	...
	public static Object valueForBinding(String name, Object defaultValue, NSDictionary<String, WOAssociation> associations, WOComponent component) {
		Object value = AjaxPTUtils.valueForBinding(name, associations, component);
		if (value != null) {
			return value;
		}
		return defaultValue;
	...
	/*
	 * @deprecated use {@link er.extensions.components.ERXComponentUtilities#stringValueForBinding(String, String, NSDictionary, WOComponent)}
	 *             instead
	 */
	@Deprecated
	public static String stringValueForBinding(String name, String defaultValue, NSDictionary<String, WOAssociation> associations,
			WOComponent component) {
		String value = AjaxPTUtils.stringValueForBinding(name, associations, component);
		if (value != null) {
			return value;
		}
		return defaultValue;
	...
	/*
	 * @deprecated use {@link er.extensions.components.ERXComponentUtilities#booleanValueForBinding(String, boolean, NSDictionary, WOComponent)}
	 *             instead
	 */
	@Deprecated
	public static boolean booleanValueForBinding(String name, boolean defaultValue, NSDictionary<String, WOAssociation> associations,
			WOComponent component) {
		WOAssociation association = associations.objectForKey(name);
		if (association != null) {
	...
	/*
	 * @deprecated use {@link er.extensions.components.ERXComponentUtilities#setValueForBinding(Object, String, NSDictionary, WOComponent)}
	 *             instead
	 */
	@Deprecated
	 ...
	 *                the array type
	 * @param component
	 *                the component to resolve against
	 * @param association
	 *                the association to retrieve a value for
	 * @return an array (or null)
	 */
// XXX schmied	public static <T> NSArray<T> arrayValueForAssociation(WOComponent component, WOAssociation association) {
//		NSArray<T> array = null;
//		if (association != null) {
//			array = AjaxPTUtils.arrayValueForObject(association.valueInComponent(component));
//		}
//		return array;
//	}
	 ...
	 * @return an array (or null)
	 */
// XXX schmied	public static <T> NSArray<T> arrayValueForBinding(WOComponent component, String bindingName) {
//		return AjaxPTUtils.arrayValueForObject(component.valueForBinding(bindingName));
//	}
	 ...
	 * @return an array (or null)
	 */
// XXX schmied	@SuppressWarnings("unchecked")
...
//	}
	...
	public static String ajaxComponentActionUrl(WOContext context) {
		...
		if (AjaxPTRequestHandler.useAjaxRequestHandler()) {
			actionUrl = actionUrl.replaceFirst("/" + WOApplication.application().componentRequestHandlerKey() + "/", "/"
					+ AjaxPTRequestHandler.AjaxRequestHandlerKey + "/");
		}
		return actionUrl;
	}

	public static void appendTagAttributeAndValue(WOResponse response, WOContext context, WOComponent component,
			NSDictionary<String, WOAssociation> associations, String name) {
		AjaxPTUtils.appendTagAttributeAndValue(response, context, component, associations, name, null);
	}

	public static void appendTagAttributeAndValue(WOResponse response, WOContext context, WOComponent component,
			NSDictionary<String, WOAssociation> associations, String name, String appendValue) {
		AjaxPTUtils.appendTagAttributeAndValue(response, context, component, name, associations.objectForKey(name), appendValue);
	}

	public static void appendTagAttributeAndValue(WOResponse response, WOContext context, WOComponent component, String name,
			WOAssociation association) {
		AjaxPTUtils.appendTagAttributeAndValue(response, context, component, name, association, null);
	}

	public static void appendTagAttributeAndValue(WOResponse response, WOContext context, WOComponent component, String name,
			WOAssociation association, String appendValue) {
	 ...
	 * 
	 * @param component
	 *                full page WOComponent instance to redirect to
	 */
	public static void redirectTo(WOComponent component) {
		...
		redirect.appendToResponse(AjaxPTUtils.createResponse(context.request(), context), context);
	}
	 ...
	 */
	public static void updateDomElement(WOResponse response, String id, Object value, String numberFormat, String dateFormat,
		...
		response.appendContentString("Element.update('" + id + "'," + AsyncRRValue.javaScriptEscaped(value) + ");");
	}
}
